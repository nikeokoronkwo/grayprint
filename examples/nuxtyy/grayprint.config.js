...
export default {
    ...
    options: [{
        ...
        type: 'list',
        multiple: true,
        options: Object.entries(moduleMap).map(([k, v]) => ({ title: k, description: v }))
    }],
    ...
    create: async (context) => {
        ...
        await context.packages.exec(
            'nuxi', 'init', 
            '--gitInit', '--packageManager', context.packageManager, 
            '-M', modules.map(m => moduleMap[m]).join(','), context.path.ROOT);
