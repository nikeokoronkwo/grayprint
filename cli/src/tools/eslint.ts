...
export default [
  {files: ["**/*.{${
      [
        ...
        "cjs",
        "ts",
        ...(context.options?.typescript ? ["ts"] : []),
        ...(context.options?.vue
          ? ["vue"]
          : (context.options?.react ? ["tsx", "jsx"] : [])),
      ].join(",")
    }}"]},
  {languageOptions: ${
      context.options?.browser || context.options?.browser
        ? `{ globals: {${
          context.options?.browser ? "...globals.browser," : ""
        } ${context.options?.server ? "...globals.node" : ""}} }},`
        : "},"
    }
  ...
  ${context.options?.vue ? '...pluginVue.configs["flat/essential"],' : ""}
    ...
    context.writeFile(
      "eslint.config.js",
      prettier.format(configChunks.join("\n"), { parser: "babel" }),
    );
  },
});
