  ...
  private _runCommand(args: string[], exec: boolean = false) {
    return new Promise<boolean>((resolve) => {
      const { status: code } = spawnSync(
        exec ? this.execName ?? this.name : this.name,
        args,
...
const denoCommands = (cwd: string) =>
  new TemplatePkgManagerInterface({
    name: "deno",
    add: ["add"],
    remove: ["remove"],
    install: ["install"],
