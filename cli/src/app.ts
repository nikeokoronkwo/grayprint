...
export class Application<T extends TemplateConfig = TemplateConfig> implements TemplateBuiltContext {
    ...
    static fromContext(context: TemplateContext, options?: {
        templateType?: TemplateType
    }): Application {
        return new Application({
            typescript: Object.keys(context.config).includes('typescript') ? context.config['typescript'] : false,
    ...
    question(q: TemplateOptions): PromiseLike<string | boolean | string[]> {
        throw new Error("Cannot use question query on built context. Run on `beforeCreate` to fix this error.");
    }
